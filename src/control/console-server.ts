...
let lastSig='';
async function post(url,body){await fetch(url,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});lastSig='';await refresh()}
async function refresh(){const items=await (await fetch('/api/interventions')).json();
// Re-render only when the queue actually changes. A blind 1.5s innerHTML rewrite
// destroyed the operator-identity input mid-typing, so control could never be taken.
const sig=JSON.stringify(items.map(i=>[i.id,i.status]));if(sig===lastSig)return;lastSig=sig;
queue.innerHTML=items.length?'':'<div class="card">No interventions waiting.</div>';for(const item of items){const el=document.createElement('section');el.className='card';el.innerHTML='<span class="badge">'+item.status+'</span><h2>'+item.capability+' · '+item.step+'</h2><p><strong>Intent:</strong> '+item.intent+'</p><p><strong>Why paused:</strong> '+item.reason+'</p><p><strong>Requested:</strong> '+item.requestedAction+'</p><pre>'+JSON.stringify(item,null,2)+'</pre>';if(item.screenshot)el.innerHTML+='<img src="'+item.screenshot+'">';if(item.status==='waiting'){const inp=document.createElement('input');inp.placeholder='operator identity';inp.value='operator@interface.test';const b=document.createElement('button');b.textContent='Take control';b.onclick=()=>{const operator=inp.value.trim();if(operator)post('/api/interventions/'+item.id+'/take',{operator})};el.appendChild(inp);el.appendChild(b)}if(item.status==='human_control'){const b=document.createElement('button');b.textContent='Hand back';b.onclick=()=>post('/api/interventions/'+item.id+'/hand-back',{});el.appendChild(b)}queue.appendChild(el)}}
