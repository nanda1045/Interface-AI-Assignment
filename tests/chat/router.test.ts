...
// A fetch that returns one canned Anthropic Messages response, so the router is
// tested without a model call.
function modelReturning(content: unknown[]): typeof fetch {
  return (async () => new Response(JSON.stringify({ content }), { status: 200, headers: { "content-type": "application/json" } })) as unknown as typeof fetch;
}
...
describe("chat router", () => {
  it("returns a validated capability call keeping only declared inputs", async () => {
    const fetchImpl = modelReturning([{ type: "tool_use", id: "t1", name: "find_members_by_last_name", input: { last_name: "Turing", extra: "drop me" } }]);
  ...
  it("surfaces a clarification request as its own outcome", async () => {
    const fetchImpl = modelReturning([{ type: "tool_use", id: "t1", name: "request_clarification", input: { message: "Which last name?" } }]);
  ...
  it("surfaces an unsupported report as its own outcome", async () => {
    const fetchImpl = modelReturning([{ type: "tool_use", id: "t1", name: "report_unsupported", input: { message: "I can't reset passwords." } }]);
  ...
  it("fails closed when the model answers in prose instead of calling a tool", async () => {
    const fetchImpl = modelReturning([{ type: "text", text: "Sure, the balance is probably fine." }]);
    const decision = await routeQuestion({ ...base, fetchImpl });
    expect(decision.kind).toBe("unsupported");
  });

  it("fails closed when the model names a capability outside the catalog", async () => {
    const fetchImpl = modelReturning([{ type: "tool_use", id: "t1", name: "delete_everything", input: {} }]);
